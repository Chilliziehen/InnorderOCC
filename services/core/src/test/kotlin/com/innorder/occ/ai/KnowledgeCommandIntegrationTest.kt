...
class KnowledgeCommandIntegrationTest {
    ...
    fun reset() {
        ...
        try {
            admin.update("UPDATE ai.knowledge_document SET current_version = 1, state = 'READY', row_version = 7 WHERE id = ?", DOCUMENT_ID)
        } finally {
    ...
    fun `expected version and concurrent activation permit exactly one mutation`() {
        ...
        assertThat(results.count { it.exceptionOrNull() is OptimisticConflictException }).isEqualTo(1)
    ...
    companion object {
        ...
        fun initializeDatabase() {
            ...
            listOf(PRINCIPAL_ID, DOCUMENT_ID, SOURCE_ID, PROVIDER_ID).forEachIndexed { index, id -> db.update("INSERT INTO authz.entity(id, entity_type_id, entity_type_version_id, entity_key, state) VALUES (?, ?, ?, ?, 'ACTIVE')", id, typeId, typeVersionId, "knowledge:$index") }
            ...
            db.update("INSERT INTO ai.knowledge_document(id, source_id, document_key, state, row_version) VALUES (?, ?, 'document', 'READY', 7)", DOCUMENT_ID, SOURCE_ID)
            db.update("INSERT INTO ai.knowledge_document_version(id, document_id, version, object_key, content_hash, mime_type, parser_version, data_classification) VALUES (?, ?, 1, 'q/old', ?, 'text/plain', 'v1', 'INTERNAL'), (?, ?, 2, 'q/new', ?, 'text/plain', 'v1', 'INTERNAL')", OLD_VERSION_ID, DOCUMENT_ID, OLD_HASH, NEW_VERSION_ID, DOCUMENT_ID, NEW_HASH)
            db.update("UPDATE ai.knowledge_document SET current_version = 1 WHERE id = ?", DOCUMENT_ID)
            ...
            val documentManifest = "$NEW_VERSION_ID:$NEW_HASH"
            db.update("INSERT INTO ai.embedding_space_gate_evaluation(id, dataset_version_id, dataset_content_hash, corpus_manifest_digest, document_manifest, candidate_embedding_space_id, expected_active_space_id, eligible_count, embedded_count, leakage_count, evidence_hash) VALUES (?, ?, ?, ?, ?, ?, ?, 1, 1, 0, ?)", GATE_ID, DATASET_VERSION_ID, "4d".repeat(32), MANIFEST, documentManifest, CANDIDATE_SPACE_ID, ACTIVE_SPACE_ID, "5e".repeat(32))
            db.update("INSERT INTO ai.embedding_space_gate_result(id, dataset_version_id, dataset_content_hash, corpus_manifest_digest, document_manifest, candidate_embedding_space_id, expected_active_space_id, eligible_count, embedded_count, leakage_count, citation_numerator, citation_denominator, recall_sum, recall_count, decision, evidence_hash) VALUES (?, ?, ?, ?, ?, ?, ?, 1, 1, 0, 20, 20, 20, 20, 'PASS', ?)", GATE_ID, DATASET_VERSION_ID, "4d".repeat(32), MANIFEST, documentManifest, CANDIDATE_SPACE_ID, ACTIVE_SPACE_ID, "5e".repeat(32))
        }
