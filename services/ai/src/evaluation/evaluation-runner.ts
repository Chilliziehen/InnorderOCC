...
type Evidence = Readonly<{ supportedCitations: number; totalCitations: number; relevantFoundAt10: number; relevantExpected: number; outcome: string; evidenceHash: string }>;
...
export class EvaluationRunner {
  ...
  async run(signal: AbortSignal): Promise<Readonly<{ evaluationId: string; decision: "PASS" | "FAIL" }>> {
    ...
    for (const item of dataset.cases) {
      ...
      if (!Number.isSafeInteger(evidence.supportedCitations) || !Number.isSafeInteger(evidence.totalCitations) || evidence.totalCitations < 1 || evidence.supportedCitations < 0 || evidence.supportedCitations > evidence.totalCitations || !Number.isSafeInteger(evidence.relevantFoundAt10) || !Number.isSafeInteger(evidence.relevantExpected) || evidence.relevantExpected < 1 || evidence.relevantFoundAt10 < 0 || evidence.relevantFoundAt10 > Math.min(10, evidence.relevantExpected) || !HASH.test(evidence.evidenceHash)) throw new Error("OCC-AI-EVALUATION-EVIDENCE");
      if (evidence.outcome !== item.expectedProperties.expectedOutcome) throw new Error(item.expectedProperties.adversarial ? "OCC-AI-EVALUATION-ADVERSARIAL" : "OCC-AI-EVALUATION-OUTCOME");
      await this.dependencies.repository.recordCase({ evaluationId: persistedEvaluationId, caseId: item.id, citationNumerator: evidence.supportedCitations, citationDenominator: evidence.totalCitations, recallAt10: evidence.relevantFoundAt10 / evidence.relevantExpected, evidenceHash: evidence.evidenceHash }, signal);
    }
