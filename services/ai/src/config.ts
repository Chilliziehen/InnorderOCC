...
const ConfigSchema = z
  .object({
    ...
    ingestionEnabled: BooleanSchema.default(false),
    parserInputRoot: z.string().min(1).max(1024).optional(),
    parserRequestRoot: z.string().min(1).max(1024).optional(),
    parserOutputRoot: z.string().min(1).max(1024).optional(),
    parserTimeoutMs: z.coerce.number().int().min(1).max(60_000).default(60_000),
    parserPollMs: z.coerce.number().int().min(5).max(1_000).default(25),
    parserHeartbeatMaxAgeMs: z.coerce.number().int().min(1_000).max(60_000).default(10_000),
  })
  .strict()
  .superRefine((config, context) => {
    if (config.ingestionEnabled) for (const key of ["parserInputRoot", "parserRequestRoot", "parserOutputRoot"] as const) {
      if (config[key] === undefined) context.addIssue({ code: "custom", path: [key], message: "Required when ingestion is enabled" });
    }
...
export function loadConfig(
  ...
  const raw = {
    ...
    ingestionEnabled: environment.AI_INGESTION_ENABLED,
    parserInputRoot: environment.AI_PARSER_INPUT_ROOT,
    parserRequestRoot: environment.AI_PARSER_REQUEST_ROOT,
    parserOutputRoot: environment.AI_PARSER_OUTPUT_ROOT,
    parserTimeoutMs: environment.AI_PARSER_TIMEOUT_MS,
    parserPollMs: environment.AI_PARSER_POLL_MS,
    parserHeartbeatMaxAgeMs: environment.AI_PARSER_HEARTBEAT_MAX_AGE_MS,
  };
