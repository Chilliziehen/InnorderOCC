...
test('fresh PostgreSQL applies V001 through V014 and passes the workflow SQL contract', { timeout: 180_000 }, async () => {
  const migrations = readdirSync(migrationRoot).filter((name) => /^V\d+__.*\.sql$/u.test(name)).sort();
  assert.deepEqual(migrations.map((name) => name.slice(0, 4)), Array.from({ length: 14 }, (_, index) => `V${String(index + 1).padStart(3, '0')}`));
