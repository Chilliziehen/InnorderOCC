...
// The platform/evidence surface spells codes with hyphens and the workflow
// surface with underscores. Both are live wire contracts, so the aliases below
// keep the original platform-side names working against the layered schema.
export const OCC_PROBLEM_CODES = PLATFORM_PROBLEM_CODES;
export const occProblemCodeSchema = platformProblemCodeSchema;
export type OccProblemCode = z.infer<typeof platformProblemCodeSchema>;
