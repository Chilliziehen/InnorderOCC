...
运行配置只允许 `infra/compose/.env` 保存十个绝对文件路径、必填 JWT issuer 和十二个可选非敏感覆盖。检查进程和 Compose 时不使用会展开敏感环境/挂载的完整 `docker inspect`，也不收集主机或容器进程命令行；普通支持包不包含 Compose `config`，因为它会暴露主机密钥路径。该限制是当前 Redis argv残余风险的必要补偿控制，不代表管理员无法读取 argv。
