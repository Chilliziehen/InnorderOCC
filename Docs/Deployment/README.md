...
当前部署单元是 `infra/compose/compose.yml` 定义的每客户独立、单主机 Compose 栈。手册覆盖主机预检、容量初始基线、九个文件型密钥、配置、Windows/Linux 生命周期、状态检查、备份恢复、升级回滚、事件处置和安全加固。
当前部署单元是 `infra/compose/compose.yml` 定义的每客户独立、单主机 Compose 栈。手册覆盖主机预检、容量初始基线、十个长期文件型密钥、一次性管理员引导文件、配置、Windows/Linux 生命周期、状态检查、备份恢复、升级回滚、事件处置和安全加固。
...
1. 按第 03 章生成十一个互异标量密钥、一对匹配的 JWT PEM 文件和受限的一次性管理员引导文件。
...
4. 确认三个一次性任务成功完成，不把已退出且退出码为零误判为故障。

### 验收

1. 检查十一个服务的状态、健康语义和八个回环监听。
