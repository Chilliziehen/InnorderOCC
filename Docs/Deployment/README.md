...
当前部署单元是 `infra/compose/compose.yml` 定义的每客户独立、单主机 Compose 栈。手册覆盖主机预检、容量初始基线、十个文件型密钥、配置、Windows/Linux 生命周期、状态检查、备份恢复、升级回滚、事件处置和安全加固。
...
1. 按第 03 章生成八个互异标量密钥和一对匹配的 JWT PEM 文件。
